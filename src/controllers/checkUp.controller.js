...
export async function submitRegister(req, res) {
  ...
  try {
    ...
    const result_check = await query(
      "SELECT * FROM checkupregister WHERE id = $1 AND status = $2",
      [id, "PENDING"]
    );
    ...
    if (result_check.rowCount === 0) {
      return res
        ...
        .json({
          error: true,
          message: "Không có tồn tại Register or đã CANCEL",
        });
    }
...
export async function getCampaignDetail(req, res) {
  ...
  try {
    ...
    const rs = await query(
      `SELECT c.id AS campaign_id, c.name AS campaign_name,sel.id, sel.name AS spe_name, sel.description
    ...
    if (rs.rowCount === 0) {
      return res
        .status(400)
        .json({ error: true, message: "Không lấy được Campaign." });
    } else {
      return res
        .status(200)
        .json({ error: false, message: "Lấy Details Campaign thành công" });
    }
  } catch (err) {
    console.error("❌ Error creating Campaign ", err);
    return res

  ...
  try {
    ...
    if (rs.rowCount === 0) {
      return res
        ...
        .json({
          error: true,
          message: "Tạo mới Specialist Exam không thành công",
        });
    }
  ...
  try {
    if (!id || !name || !description) {
      return res
        ...
        .json({
          error: true,
          message: "Không Nhận được ID or name or description",
        });
    }
...
export async function deleteSpecialExam(req, res) {
  const { id } = req.params;
  ...
  try {
    ...
    const check = await query(`SELECT * FROM specialistexamlist WHERE id = $1`, [id]);

    if (check.rowCount === 0) {
      return res.status(400).json({ error: true, message: "Chuyên khoa không tồn tại" });
    }

    const rs = await query(`UPDATE specialistexamlist SET is_deleted = TRUE WHERE id = $1`, [id]);
    ...
    console.error("❌ Error deleting specialist exam:", err);
    return res.status(500).json({ error: true, message: "Lỗi server khi xóa chuyên khoa" });
  }
}